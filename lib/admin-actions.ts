...
export async function adminDeleteUser(userId: string): Promise<ActionResult> {
  ...
  if (profile.id === userId) return { success: false, error: 'You cannot delete your own account.' };
...
export async function adminImportUsers(params: {
  rows: Array<{
    name: string;
    email: string;
    phone?: string | null;
  ...
  const profile = await getCurrentProfile();
  if (!profile) return { success: false, error: 'Not authenticated' };
  if (profile.role !== 'ADMIN') return { success: false, error: 'Only admins can import users.' };

  if (params.defaultPassword.length < 6) {
    return { success: false, error: 'Default password must be at least 6 characters.' };
  }
  ...
  for (let i = 0; i < params.rows.length; i += BATCH_SIZE) {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        action: 'import_users',
        import_rows: batch,
        default_password: params.defaultPassword,
      }),
    });
    ...
    if (json.error) return { success: false, error: json.error };
    if (json.summary) {
      combinedSummary.total_rows += json.summary.total_rows || 0;
      combinedSummary.accounts_created += json.summary.accounts_created || 0;
      combinedSummary.entries_imported += json.summary.entries_imported || 0;
      combinedSummary.entries_skipped += json.summary.entries_skipped || 0;
      combinedSummary.unmatched_categories += json.summary.unmatched_categories || 0;
      combinedSummary.duplicates += json.summary.duplicates || 0;
    }
  ...
  return { success: true, summary: combinedSummary, errors: allErrors };
}
